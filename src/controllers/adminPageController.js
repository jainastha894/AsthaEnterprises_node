...
const addProduct = async (req, res) => {
  try {
    ...
    // 1️⃣ Basic fields
    const { productName, description, hsnCode } = req.body;
    console.log("product name is ", productName);
    console.log('Product HSN Code is:', hsnCode);

    if (!productName || !description || !hsnCode) {
      console.error('Validation failed: Product name, description or HSN code missing');
      return res.redirect("/admin/products/add?error=" + encodeURIComponent("Product name, description and HSN code are required"));
    }
    ...
    console.log('Product hsn code:', savedProduct.hsnCode);
...
const productListPage = async (req, res) => {
  try {
    ...
    const unitsPath = path.join(process.cwd(), "src/config/units.json");
    const unitsData = JSON.parse(fs.readFileSync(unitsPath, "utf-8"));
    console.log('ProductListPage - Fetching products from database...');
    ...
    res.render("admin/products", { products: productsArray, units: unitsData }); //mark
  } catch (error) {
...
const profilePage = async (req, res) => {
  try {
    ...
    // Get admin data
    const admin = await Admin.findOne({ username: req.user?.username || "AsthaAdmin" });
...
const profileSettingsPage = async (req, res) => {
  try {
    // Get admin data
    const admin = await Admin.findOne({ username: req.user?.username || "AsthaAdmin" });
...
const updateProfile = async (req, res) => {
  try {
    ...
    if (!admin) {
      ...
      const hashedPassword = await bcrypt.hash("atulparamjain", 10);
      admin = new Admin({
        username: "AsthaAdmin",
...
const updateProduct = async (req, res) => {
  try {
    ...
    const { productName, description, hsnCode, imagesToRemove } = req.body;

    console.log('Updating product hsn code:', hsnCode);
    ...
    console.log('Updated product hsn code:', updatedProduct.hsnCode);
...
const leadListPage = async (req, res) => {
  try {
...
const leadDetailPage = async (req, res) => {
  try {
    const { id } = req.params;
...
const productEnquiriesPage = async (req, res) => {
  try {
